...
class ServiceInfoViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let policyTap = UITapGestureRecognizer(target: self, action: #selector(gopolicy))

        introduceView.addGestureRecognizer(introduceTap)
    ...
    @objc func gointroduce(){
        let vc = UIStoryboard(name: "Mypage", bundle: nil).instantiateViewController(withIdentifier: "ServiceDetailViewController") as! ServiceDetailViewController
        vc.titlevalue = "DMZING 안내"
        vc.contentvalue = """
        제1조 목적

        본 이용약관은 “DMZING”(이하 "사이트")의 서비스의 이용조건과 운영에 관한 제반사항 규정을 목적으로 합니다.
